...
这部分waline的初始化代码中添加`dark`参数，然后将其值设定为`true`即可。
```javascript
dark: true
```
...
| --- | --- | --- |
| `"html[data-mode=dark]"` | <span style="color: orange"> dark </span> | <span style="color: orange"> **dark** </span> |
...
| `"html:not([data-mode])"` | <span style="color: salmon"> light </span> | <span style="color: salmon"> **dark** </span> |
...
那么想要实现评论区主题的自动切换就需要写一套比较复杂的逻辑，而考虑到本人对于前端技术的理解及其有限，所以最终采取了一套更为粗暴的解决方案，即**直接在博客设定的css样式中定义不同主题下waline评论区的样式**。而关于waline的自定义样式，官网也给出了详细的说明（参考 **[此处](https://waline.js.org/reference/client/style.html)**）。
...
- [waline CSS变量](https://waline.js.org/reference/client/style.html)
